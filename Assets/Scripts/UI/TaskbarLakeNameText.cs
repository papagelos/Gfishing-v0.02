...
public class TaskbarLakeNameText : MonoBehaviour
{
    ...
    private void TryAutoDiscoverWorldManager()
    {
        ...
        bool usedEmergencyDock;
        worldManager = FindWorldManagerOmni(out usedEmergencyDock);
        if (!worldManager) return;
        ...
        if (usedEmergencyDock)
            Debug.Log($"[UI] {gameObject.name} Emergency Dock -> WorldManager '{worldManager.gameObject.name}'.");
    }

    private static WorldManager FindWorldManagerOmni(out bool usedEmergencyDock)
    {
        usedEmergencyDock = false;

        // Primary (safe) search: works for active and inactive scene objects.
        var manager = UnityEngine.Object.FindAnyObjectByType<WorldManager>(FindObjectsInactive.Include);
        if (manager != null)
            return manager;

        // Emergency search: includes hidden / DontDestroyOnLoad objects.
        var allManagers = Resources.FindObjectsOfTypeAll<WorldManager>();
        ...
        for (int i = 0; i < allManagers.Length; i++)
        {
            ...
            // Skip assets/prefab objects; keep scene instances.
            if (!candidate.gameObject.scene.IsValid()) continue;

            usedEmergencyDock = true;
            return candidate;
        }
